...
CREATE INDEX IF NOT EXISTS idx_orcamentos_paciente ON public.orcamentos(paciente_id);
CREATE INDEX IF NOT EXISTS idx_orcamentos_prontuario ON public.orcamentos(prontuario_id);
...
CREATE INDEX IF NOT EXISTS idx_faturas_prontuario ON public.faturas(prontuario_id);
CREATE INDEX IF NOT EXISTS idx_faturas_status ON public.faturas(status);
CREATE INDEX IF NOT EXISTS idx_faturas_data ON public.faturas(data_emissao);
CREATE INDEX IF NOT EXISTS idx_faturas_numero ON public.faturas(numero_nf);
...
CREATE INDEX IF NOT EXISTS idx_pagamentos_fatura ON public.pagamentos(fatura_id);
CREATE INDEX IF NOT EXISTS idx_pagamentos_data ON public.pagamentos(data_pagamento);
CREATE INDEX IF NOT EXISTS idx_pagamentos_metodo ON public.pagamentos(metodo_pagamento);
...
CREATE INDEX IF NOT EXISTS idx_cupons_ativo ON public.cupons_desconto(ativo);
CREATE INDEX IF NOT EXISTS idx_cupons_validade ON public.cupons_desconto(validade_fim);
...
CREATE INDEX IF NOT EXISTS idx_cupom_uso_cupom ON public.cupom_uso(cupom_id);
CREATE INDEX IF NOT EXISTS idx_cupom_uso_fatura ON public.cupom_uso(fatura_id);
...
CREATE POLICY "Serviços editáveis por admin" ON public.servicos
  FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Serviços Update por admin" ON public.servicos
  FOR UPDATE
  USING (auth.role() = 'authenticated');

-- RLS Policies: Orçamentos
CREATE POLICY "Orçamentos visualizáveis por autenticados" ON public.orcamentos
  FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "Orçamentos criáveis por dentista/recepção" ON public.orcamentos
  FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Orçamentos Update por criador/admin" ON public.orcamentos
  FOR UPDATE
  USING (auth.role() = 'authenticated');

CREATE POLICY "Orçamentos deletáveis por criador" ON public.orcamentos
  FOR DELETE
  USING (auth.role() = 'authenticated');

-- RLS Policies: Itens do Orçamento (herdar da fatura)
...
CREATE POLICY "Faturas visualizáveis por autenticados" ON public.faturas
  FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "Faturas criáveis por dentista/recepção" ON public.faturas
  FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Faturas Update por criador/admin" ON public.faturas
  FOR UPDATE
  USING (auth.role() = 'authenticated');
...
CREATE POLICY "Pagamentos visualizáveis por autenticados" ON public.pagamentos
  FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "Pagamentos registráveis por dentista/recepção" ON public.pagamentos
  FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

-- RLS Policies: Cupons
CREATE POLICY "Cupons visualizáveis por autenticados" ON public.cupons_desconto
  FOR SELECT
  USING (auth.role() = 'authenticated' AND ativo = true);

CREATE POLICY "Cupons criáveis por admin" ON public.cupons_desconto
  FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Cupons Update por admin" ON public.cupons_desconto
  FOR UPDATE
  USING (auth.role() = 'authenticated');

-- RLS Policies: Cupom Uso (log)
